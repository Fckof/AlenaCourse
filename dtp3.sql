...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Апр 18 2022 г., 20:29
...
FROM `Водители` INNER JOIN `участники_дтп` ON `участники_дтп`.`ID_водителя` = `водители`.`ID_водителя`$$

DROP PROCEDURE IF EXISTS `Водители_в_ДТП_колво`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Водители_в_ДТП_колво` ()  SELECT `Водители`.`Фамилия`, `Водители`.`Имя`, `Водители`.`Отчество`, Count(`участники_дтп`.`ID_дтп`) AS `ДТП`
FROM `Водители` INNER JOIN `участники_дтп` ON `участники_дтп`.`ID_водителя` = `Водители`.`ID_водителя`
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ДТП_с_пострадавшими` ()  SELECT `дтп`.`Дата`, `дтп`.`Место`, (`дтп`.`Пострадало` + `дтп`.`Погибло`) as Пострадали FROM `дтп` WHERE (`дтп`.`Пострадало` + `дтп`.`Погибло`) <> 0$$

DROP PROCEDURE IF EXISTS `обновить_номер_авто`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `обновить_номер_авто` (IN `id_авто` INT, IN `номер` VARCHAR(10))  UPDATE `авто` SET `авто`.`Номер_гос_рег`=номер WHERE `авто`.`ID_авто`=id_авто$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Отчет_сколько_пострадало_в_ДТП_за_опр_год` ()  SELECT Year(`Дата`) AS `Год`, sum(`Пострадало`) as `пострадало` , sum(`Погибло`) as `погибло`  FROM `дтп`  GROUP BY `Год` ORDER by `Год`$$
...
ORDER BY Year(`Авто`.`Дата_постановки_на_учет`)$$

DROP PROCEDURE IF EXISTS `Сколько_авто_участ_в_ДТП`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Сколько_авто_участ_в_ДТП` ()  SELECT `Марка`.`Марка`, Count(`участники_дтп`.`ID_дтп`) AS `ДТП`
FROM `Марка` INNER JOIN `Авто` ON `Марка`.`ID_марки` = `Авто`.`ID_марки` INNER JOIN `участники_дтп` ON `Авто`.`ID_авто` = `участники_дтп`.`ID_машины`
...
CREATE TABLE IF NOT EXISTS `марка` (
  ...
  `Марка` varchar(13) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `дтп` (
  `ID_дтп` int NOT NULL AUTO_INCREMENT,
  `Дата` date NOT NULL,
  `Место` varchar(57) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  `Пострадало` tinyint NOT NULL,
  `Погибло` tinyint NOT NULL,
  ...
  PRIMARY KEY (`ID_дтп`),
  KEY `ID_водителя` (`ID_лиц_проводившего_расслед`)
...
INSERT INTO `дтп` (`ID_дтп`, `Дата`, `Место`, `Пострадало`, `Погибло`, `Описание`, `ID_лиц_проводившего_расслед`) VALUES
(1, '2021-08-06', 'Тверская область, город Щёлково, спуск Домодедовская, 47', 5, 2, 'Водитель Audi выехал на встречную полосу и протаранил несколько автомобилей', 1),
(2, '2021-09-24', 'Оренбургская область, город Лотошино, пер. Ломоносова, 25', 3, 1, 'Водитель BMW совершил несколько опасных обгонов и в итоге задел грузовик', 2),
(3, '2019-05-15', 'Курская область, город Кашира, въезд Ленина, 44', 2, 3, 'Автомобиль на скорости 200 кмч сбил пешеходов на красный свет светофора после чего врезался в столб', 3),
(4, '2021-08-09', 'Тверская область, город Раменское, ул. Ломоносова, 12', 1, 0, 'У машины отказали тормоза в следствие чего автомобиль не мог совершить остановку и въехал в другой автомобиль', 4),
(5, '2020-01-18', 'Тверская область, город Кашира, пр. Ломоносова, 93', 0, 0, 'Автомобиль выезжая с парковки задел движущийся автомобиль', 5),
(6, '2022-03-27', 'Нижегородская область, город Наро-Фоминск, пр. Чехова, 30', 2, 0, 'Водитель не смог удержать управление на скользкой дороге и врезался в отбойник', 3),
(7, '2022-02-03', 'Челябинская область, город Воскресенск, пл. Ленина, 13', 0, 0, 'У автомобиля загорелся двигатель после того как в него врезался Мерседес', 1);
...
CREATE TABLE IF NOT EXISTS `пользователи` (
  ...
  `login` varchar(10) COLLATE utf8mb4_general_ci NOT NULL,
...
INSERT INTO `пользователи` (`id`, `login`, `pass`, `role`) VALUES
(1, 'admin', 'admin123', 'ADMIN'),
(2, 'worker', 'worker123', 'WORKER'),
(3, 'user', 'user123', 'GUEST');
...
CREATE TABLE IF NOT EXISTS `авто` (
  ...
  `Номер_гос_рег` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
...
(1, 4, 2, 'R450HH553', 'BZ7LB', 565039, 582421, '2736 690603', '2016-01-16', '1997-04-23'),
(2, 4, 5, 'R450HH540', 'VE7SN', 33928, 274029, '6574 800145', '2013-03-18', '1995-08-11'),
...
-- Триггеры `авто`
--
DROP TRIGGER IF EXISTS `обновить_номер_авто`;
DELIMITER $$
CREATE TRIGGER `обновить_номер_авто` AFTER UPDATE ON `авто` FOR EACH ROW IF (old.`Номер_гос_рег` <> new.`Номер_гос_рег`) then
SET @old_date=(SELECT max(`Дата_после`) as latest FROM `смена_номера_машины` WHERE `id_авто`= new.`ID_авто`);
IF @old_date is null THEN
	SET @old_date=(SELECT `Дата_постановки_на_учет` FROM `авто` WHERE `авто`.`ID_авто`=new.`ID_авто`);
end IF;
 INSERT INTO `смена_номера_машины`(`id_авто`,`Дата_до`,`Дата_после`,`Старый_номер`,`Новый_номер`) VALUES(new.`id_авто`,@old_date,NOW(),old.`Номер_гос_рег`,new.`Номер_гос_рег`);
 end if
$$
DELIMITER ;
DROP TRIGGER IF EXISTS `обновить_цвет_авто`;
DELIMITER $$
CREATE TRIGGER `обновить_цвет_авто` AFTER UPDATE ON `авто` FOR EACH ROW IF (old.`ID_цвета` <> new.`ID_цвета`) then
SET @old_date=(SELECT max(`Дата_после`) as latest FROM `смена_цвета_машины` WHERE `id_авто`= new.`ID_авто`);
SET @old_color=(SELECT `цвет авто`.`Цвет` FROM `цвет авто` WHERE `цвет авто`.`ID_цвета`=old.`ID_цвета`);
SET @new_color=(SELECT `цвет авто`.`Цвет` FROM `цвет авто` WHERE `цвет авто`.`ID_цвета`=new.`ID_цвета`);
IF @old_date is null THEN
	SET @old_date=(SELECT `Дата_постановки_на_учет` FROM `авто` WHERE `авто`.`ID_авто`=new.`ID_авто`);
end IF;
 INSERT INTO `смена_цвета_машины` VALUES(new.`id_авто`,@old_date,NOW(),@old_color,@new_color);
 end if
$$
DELIMITER ;
...
--
-- Структура таблицы `смена_номера_машины`
--

DROP TABLE IF EXISTS `смена_номера_машины`;
CREATE TABLE IF NOT EXISTS `смена_номера_машины` (
  `id_авто` int NOT NULL,
  `Дата_до` datetime NOT NULL,
  `Дата_после` datetime NOT NULL,
  `Старый_номер` varchar(8) COLLATE utf8mb4_general_ci NOT NULL,
  `Новый_номер` varchar(8) COLLATE utf8mb4_general_ci NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Дамп данных таблицы `смена_номера_машины`
--

INSERT INTO `смена_номера_машины` (`id_авто`, `Дата_до`, `Дата_после`, `Старый_номер`, `Новый_номер`) VALUES
(1, '1997-04-23 00:00:00', '2022-04-18 20:45:33', 'Р301КА67', 'Р301КА68'),
(1, '2022-04-18 20:45:33', '2022-04-18 20:52:44', 'Р301КА68', 'Р305КА68'),
(1, '2022-04-18 20:52:44', '2022-04-18 21:37:29', 'Р305КА68', 'к564ен77'),
(1, '2022-04-18 21:37:29', '2022-04-18 22:23:35', 'к564ен77', 'R450HH54'),
(1, '2022-04-18 22:23:35', '2022-04-18 22:38:35', 'R450HH54', 'R450HH55'),
(1, '2022-04-18 22:38:35', '2022-04-18 22:39:42', 'R450HH55', 'R450HH55'),
(2, '1995-08-11 00:00:00', '2022-04-18 22:58:41', 'М534ОЕ57', '1'),
(2, '2022-04-18 22:58:41', '2022-04-18 22:59:13', '1', 'R450HH54');
...
(2, '2022-04-18 21:00:28', '2022-04-18 23:00:09', 'Оливковый', 'Синий'),
(2, '2022-04-18 23:00:09', '2022-04-18 23:00:18', 'Синий', 'Красный'),
(2, '2022-04-18 23:00:18', '2022-04-18 23:00:27', 'Красный', 'Оливковый');
...
--
-- Структура таблицы `участники_дтп`
--

DROP TABLE IF EXISTS `участники_дтп`;
CREATE TABLE IF NOT EXISTS `участники_дтп` (
  `ID_дтп` int NOT NULL,
...
--
-- Дамп данных таблицы `участники_дтп`
--

INSERT INTO `участники_дтп` (`ID_дтп`, `ID_водителя`, `ID_машины`) VALUES
(1, 3, 3),
(2, 5, 5),
(3, 20, 12),
(4, 18, 7),
...
--
ALTER TABLE `дтп`
  ADD CONSTRAINT `дтп_ibfk_1` FOREIGN KEY (`ID_лиц_проводившего_расслед`) REFERENCES `следователи` (`ID_следователя`);

--
-- Ограничения внешнего ключа таблицы `авто`
--
ALTER TABLE `авто`
  ADD CONSTRAINT `авто_ibfk_1` FOREIGN KEY (`ID_цвета`) REFERENCES `цвет авто` (`ID_цвета`),
  ADD CONSTRAINT `авто_ibfk_2` FOREIGN KEY (`ID_марки`) REFERENCES `марка` (`ID_марки`),
  ADD CONSTRAINT `авто_ibfk_3` FOREIGN KEY (`ID_авто`) REFERENCES `владельцы_авто` (`ID_авто`);

--
-- Ограничения внешнего ключа таблицы `водители`
--
ALTER TABLE `водители`
  ADD CONSTRAINT `водители_ibfk_1` FOREIGN KEY (`ID_категории`) REFERENCES `категории` (`ID_категории`);
COMMIT;
