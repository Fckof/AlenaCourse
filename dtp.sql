...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Апр 13 2022 г., 22:10
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Отчет_сколько_пострадало_в_ДТП_за_опр_год` ()  SELECT Year(`Дата`) AS `Год`, count(`Пострадало`) as "пострадало" , COUNT(`Погибло`) as "погибло"  FROM `дтп`  GROUP BY `Год` ORDER by `Год`$$
...
(2, '2021-09-24', 'Оренбургская область, город Лотошино, пер. Ломоносова, 25', 3, 0, 'Водитель BMW совершил несколько опасных обгонов и в итоге задел грузовик', 2),
(3, '2019-05-15', 'Курская область, город Кашира, въезд Ленина, 44', 1, 3, 'Автомобиль на скорости 200 кмч сбил пешеходов на красный свет светофора после чего врезался в столб', 3),
